...
export default async function HomePage() {
    ...
    return (
        <>
            ...
            {/* The server action "createProduct" is used when the form is submitted */}
            <form action={createProduct} className="my-3">
                <select name="category-id" className="me-2">
                    <option>Select category</option>
                    {categories.map(category => <option key={category.id} value={category.id}>{category.name}</option>)}
                </select>
                <input type="text" name="name" placeholder="Product name" className="me-2"/>
